...
posterior_details <- tibble(model_name = c("Basic model", "Full model"),
                            model = list(m_amount_cif,
                                         m_amount_cif_full)) %>% 
  mutate(posterior = model %>% map(~ as_tibble(.)),
         tidy_summary = model %>% map(~ tidyMCMC(., conf.int = TRUE, conf.level = 0.95,
...
amount_coefs_full <- ggplot(coefs_posterior, aes(x = value, y = fct_rev(term_clean_fct))) +
  ...
  labs(x = "Posterior median estimate + 95% credible interval", y = NULL,
       caption = "95% credible intervals shown in black. Solid vertical line = median; dotted vertical line = 0") +
...
coef_summary_charity <- newdata_conditions_charity %>% 
  bind_cols(tidyMCMC(chains_fitted_charity, estimate.method = "median",
                     conf.int = TRUE, conf.level = 0.95, conf.method = "HPDinterval")) %>% 
...
posterior_details_likely <- tibble(model_name = c("Basic model", "Full model"),
