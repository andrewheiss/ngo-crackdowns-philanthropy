...
**Just the crackdown condition**

```{r models-predicted-crackdown}
newdata_conditions <- results %>% expand(nesting(crackdown))

posterior_fit <- posterior_predict(m_amount_crackdown, 
...
ggplot(newdata, aes(y = estimate, x = crackdown)) + 
  geom_point(data = all_posterior_draws, aes(y = value, x = crackdown), 
             color = "grey50", size = 1.5, alpha = 0.25) +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) + 
  scale_y_continuous(labels = dollar) +
  labs(x = "Crackdown condition", y = "Median amount donated") +
  coord_cartesian(ylim = c(0, 100)) +
  theme_ngos()
```

**Crackdown and issue conditions**

```{r models-predicted-crackdown-issue}
newdata_conditions <- results %>% expand(nesting(crackdown, issue))
...
**Crackdown, issue, and funding conditions**

```{r models-predicted-crackdown-issue-funding}
newdata_conditions <- results %>% expand(nesting(crackdown, issue, funding))
