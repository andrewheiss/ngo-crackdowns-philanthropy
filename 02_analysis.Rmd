...
```{r causal-dag, fig.width=4.5, fig.height=2.5}
...
plot_dag <- ggplot(theory_dag, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_node(size = 12) +
  geom_dag_edges() +
  geom_dag_text(size = pts(9), family = "Encode Sans Condensed SemiBold") +
  scale_dag() +
  theme_ngos() +
...
plot_dag %T>% 
  print() %T>%
  ggsave(., filename = here("output", "figures", "causal-path.pdf"),
         width = 4.5, height = 2.5, units = "in", device = cairo_pdf) %>% 
  ggsave(., filename = here("output", "figures", "causal-path.png"),
...
```{r plot-likely-diffs, warning=FALSE, fig.width=4.5, fig.height=6.5}
...
plot_diffs_likely_all %T>% 
  ggsave(., filename = here("output", "figures", "likely-diffs.pdf"),
         width = 4.5, height = 6.5, units = "in", device = cairo_pdf) %>%
  ggsave(., filename = here("output", "figures", "likely-diffs.png"),
         width = 4.5, height = 6.5, units = "in", type = "cairo", dpi = 300)
```
...
tbl_likely_1 <- tbl_likely_tidy %>% 
  ...
  mutate_at(vars(theta_1, theta_2, theta_diff, pct_change), 
            funs(percent_format(accuracy = 0.1)(.))) %>% 
...
tbl_likely_2_header <- enframe(colnames(tbl_likely_2)) %>% 
  mutate(value = pandoc.strong.return(value)) %>% 
  spread(name, value) %>% 
...
bind_rows(tbl_likely_1, 
  ...
  pandoc.table.return(caption = 'Likelihood of donation and differences in proportions in "crackdown" (treatment) and "no crackdown" (control) conditions; values represent posterior medians {#tbl:likely-diffs}',
                      justify = "lccccc") %T>%
  cat() %>%
  cat(file = here("output", "tables", "tbl-likely-diffs.md"))
```
...
```{r plot-amount-diffs, warning=FALSE, fig.width=4.5, fig.height=6.5}
