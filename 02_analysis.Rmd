...
bind_rows(diffs_coef_pairs_detail_c,
  ...
  select(Issue = issue, Funding = funding, 
         ...
         `p(∆ > 0)` = p.greater0, `p(∆ < 0)` = p.less0, `p(∆ ≠ 0)` = p.diff.not0) %>% 
  pandoc.table.return(missing = "—", 
                      caption = 'Median difference between amounts donated in "no crackdown" (control) and "crackdown" conditions {#tbl:amount-diffs}') %T>%
  cat() %>% 
...
bind_rows(diffs_coef_likely_pairs_detail_c,
  ...
  select(Issue = issue, Funding = funding, 
         ...
         `p(∆ > 0)` = p.greater0, `p(∆ < 0)` = p.less0, `p(∆ ≠ 0)` = p.diff.not0) %>% 
  pandoc.table.return(missing = "—", 
                      caption = 'Median difference between likelihood to donate in "no crackdown" (control) and "crackdown" conditions {#tbl:likelihood-diffs}') %T>%
  cat() %>% 
...
```{r hypotheses-summary, warning=FALSE, message=FALSE, results="asis"}
# TODO: Make this more dynamic so I don't have to type all the values manually
hypotheses <- read_csv(here("text", "hypotheses-summary.csv"))

hypotheses %>% 
  pandoc.table.return(justify = "ll", style = "grid", keep.line.breaks = TRUE,
                      caption = "Summary of all hypotheses {#tbl:hypotheses}") %T>%
  cat() %>% 
  cat(file = file.path(here(), "output", "tables", "tbl-hypotheses.md"))
```
