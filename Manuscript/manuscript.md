...
thanks: >-
    We thank Mark Buntaine, Jeffrey Friedman, and Thomas Leeper for their helpful comments. We also thank Samantha Camilletti and Joshua Dutro for excellent research assistance.
...
...
As described above, while charitable giving by individual, private donors has increased over time, the decision regarding which charity to donate to is often not a major financial decision for individual donors. While a number of indicators such as Charity Navigator are available to help identify trustworthy NGOs, many donors may not scrutinize such indicators or tax documents before making their decisions [@SzperPrakash:2011; @Tremblay-BoirePrakash:2017]. Rather, they identify groups based on various other metrics such as issue area, organizational values, whether the groups pursues interests related to personal values, and seeking information from friends, family, and acquaintances [@SzperPrakash:2011; @Sloan:2009]. These other metrics and heuristics are more systemic and not necessarily connected to individual preferences. 



National contexts can also influence donor behavior. In a vignette-based survey experiment, @AndorferOtte:2012 find that donors are more likely to donate to disaster relief NGOs in countries with the greatest need, choosing to donate more to hypothetical disasters in Bangladesh and Romana than in Switzerland or Romania. 

In this study, we propose that state repression of NGOs is another structural heuristic that can influence donor behavior.
...
## Demographics and full battery of controls

TO COME LATER. This is section is mostly exploratory, though. Our main findings are based on the simple confirmatory models, and we rely on randomization to take care of differences in likelihood to donate or amount donated. In this section, we see if demographic features or attitudes towards philanthropy influence our outcomes under our different frames (i.e. are people who are more religiously active more likely to give to besieged government-funded humanitarian NGOs?). 

## Caveats

Since this survey focuses on donations by citizens rather than institutional donors, it is not reflective of the average donor dollar. However, it is important to note that Americans are driving much of this private philanthropy. As Daniel Runde pointed out in a statement before a US Senate subcommittee on multilateral institutions, "U.S. philanthropy is significant [for global philanthropic efforts] but it is mainly Ms. Smith and Mr. Johnson, [n]ot Mr. and Mrs. Gates" [-@Runde:2017].
