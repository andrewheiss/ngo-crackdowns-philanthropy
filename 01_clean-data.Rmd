...
results_raw <- read_csv(results_file, skip = 10,
...
codes_to_exclude <- readLines(file.path(here(), "data", "codes_to_exclude.txt")) %>% 
  as.numeric()
...
results <- results_raw %>%
  ...
  filter(!(confirmation_code %in% codes_to_exclude)) %>%
  # Factorize variables
